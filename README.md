...
- **压缩率上浮的余量约束（v1.3）** —— 各工况修正（静密封偏上、低压 / 浸水、高硬度、
  塑料蠕变补偿）**合计最多占用「基准 → 允许上限」余量的一半**，剩余部分留给
  热膨胀、加工公差与胶料溶胀。避免任何工况都被推到区间天花板。
- **拉伸率 / 拉伸量** —— 径向取 1%~5%，轴向按「自由中径 = 内径 + 线径」与沟槽中心
  直径对齐校核。结果页同时给出百分比与毫米增量（对槽底径或中心直径）。
- **拉伸减薄（v1.3 新增）** —— 橡胶近似体积不可压，内径撑大 δ 后线径按
  `d₂' = d₂ / √(1+δ)` 减薄，于是**实际压缩量 `c' = d₂' − h` 小于名义值 `c = d₂ − h`**。
  结果页同时列出「名义压缩量 / 实际压缩量」：校核沟槽用名义值（与标准一致），
  评估实际接触应力用实际值。δ≈4% 时两者相差约 1.4 个百分点。

### 版本

| 版本 | 变更 |
|---|---|
| **v1.3** | 压缩率修正加「至多占用至允许上限余量一半」约束（解决各工况一律顶到上限、静密封偏大的问题）；补出拉伸量（mm）、拉伸后线径与减薄系数、实际压缩量/实际压缩率；说明文档补充拉伸减薄机理 |
